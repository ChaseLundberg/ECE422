...
void __attribute__((interrupt, no_auto_psv)) _U1RXInterrupt(void){
    ...
    if (data == '1'){
        ...
        if(LED_ON == 0){
            Write('0'); //sends back 0 for LED = off
        }
        else {
            Write('1'); //sends back 1 for LED = on
        }
    }
    else if (data == '3'){
            Button += 0x30;
...
void __attribute__ ((__interrupt__, __shadow__)) _T1Interrupt(void){
        ...
        /*LATB = 0x8000 turns on Red Light */
        /*LATB = 0x2000 turns on Yellow Light */
        /*LATB = 0x1000 turns on Green Light */
...
{
    //On button press, reset interval to 0.5 seconds
    static int PR1_value = 0;
